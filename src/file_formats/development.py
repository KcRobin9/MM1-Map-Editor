...
def color_repr(color: int) -> str:
    r = (color >> 24) & 0xFF
    g = (color >> 16) & 0xFF
    b = (color >>  8) & 0xFF
    a =  color        & 0xFF
    return f"#{color:08X}  (R:{r} G:{g} B:{b} A:{a})"
...
class DLPPatch:
    ...
    def __repr__(self) -> str:
        ...
        return f"""
    Patch:
        S Resolution: {self.s_res}
        T Resolution: {self.t_res}
        Flags: {self.flags:#06x}
        Render Options: {self.r_opts:#06x}
...
class DLPGroup:
    ...
    Group:
        Name: {self.name}
        Vertex Count: {self.num_vertices}
        Patch Count: {self.num_patches}
...
class DLP:
    ...
    def __repr__(self) -> str:
        ...
        patches_str = "".join(repr(p) for p in self.patches)
