...
class VIEW3D_PT_CityLoader(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.prop(scene, "cl_load_gizmo",  text="Bridges  (.GIZMO)")
        ...
        # ── Load button ───────────────────────────────────────────────────────
        any_selected = scene.cl_load_fcd or scene.cl_load_bng or scene.cl_load_meshes or scene.cl_load_bai or scene.cl_load_gizmo
        ...
        if folder_path:
            ...
            if p.is_dir():
                ...
                gizmo_files = list(p.glob("*.GIZMO")) + list(p.glob("*.gizmo"))
                ...
                _file_row(col, "GIZMO",  gizmo_files[0].name if gizmo_files else None)
